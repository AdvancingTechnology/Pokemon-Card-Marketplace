-- Create a secure seeding function that bypasses RLS
-- This function can be called from the SQL Editor or via RPC

CREATE OR REPLACE FUNCTION seed_pokemon_data()
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  GET DIAGNOSTICS cards_inserted = ROW_COUNT;

  -- Insert Uncommon Cards
  INSERT INTO cards (name, set_name, rarity, market_price, image_url) VALUES
  ...
  SELECT COUNT(*) INTO cards_inserted FROM cards;
  ...
  SELECT COUNT(*) INTO packs_inserted FROM packs;

  -- Insert Pack Cards with weights
  ...
  SELECT COUNT(*) INTO pack_cards_inserted FROM pack_cards;

  RETURN jsonb_build_object(
    'success', true,
    'cards_count', cards_inserted,
    'packs_count', packs_inserted,
    'pack_cards_count', pack_cards_inserted
  );
END;
$$;

-- Grant execute permission
GRANT EXECUTE ON FUNCTION seed_pokemon_data() TO authenticated;
GRANT EXECUTE ON FUNCTION seed_pokemon_data() TO anon;
